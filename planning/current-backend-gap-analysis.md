...
| `ADAPT-005` Conversational/Socratic assessment | Partial | A first Socratic assessment endpoint now generates short probes and heuristically scores learner responses, but there is no persistent turn manager or richer discourse model |
...
5. `ADAPT-005` + `LLM-002`: deepen the new Socratic assessment flow into a multi-turn manager with better evidence scoring and prompt experimentation.
