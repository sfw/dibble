...
5. `ADAPT-005` + `LLM-002`: extend the new downstream-outcome calibration beyond nearby observations so the Socratic-to-profile feedback loop and adaptive prompt-selection loop can learn from richer real learner outcomes.
...
- use the new prompt-performance tracking on top of the evidence dimensions so the broader adaptive selection loop can be calibrated on richer downstream outcomes rather than immediate proxy signals
