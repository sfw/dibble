...
5. `ADAPT-005` + `LLM-002`: extend the new session-aware observation and Socratic linkage from single matched events into richer multi-step outcome traces so the Socratic-to-profile feedback loop and adaptive prompt-selection loop can learn from more trustworthy learner outcomes.
...
- use the new prompt-performance tracking and session-aware observation plus Socratic linkage so the broader adaptive selection loop can be calibrated on richer multi-step downstream outcomes rather than immediate proxy signals
- keep reusing the current generated-content and session stores instead of adding a parallel orchestration layer

That is now more achievable because the Socratic flow no longer relies only on a single last-turn threshold, no longer ends at the assessment response itself, and now feeds into both learner-state updates and same-session prompt calibration. The next coherent step is to aggregate those signals across fuller learning traces so routing and generation selection rely less on heuristics alone.
