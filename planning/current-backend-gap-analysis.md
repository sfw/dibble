...
- The backend now compacts recent run summaries plus progress snapshots into durable `learning.strategy.profile` events that expose honest heuristic long-horizon recovery and independence signals for similar future requests.
...
4. `ADAPT-005` + `LLM-002`: deepen the new learner-strategy layer beyond the current heuristic recovery/independence profiles so the Socratic-to-profile feedback loop can model plateau, relapse, and misconception recurrence across longer spans instead of mostly summarizing support direction.
...
- deepen the new learner-strategy profile beyond the current recovery and independence heuristics
