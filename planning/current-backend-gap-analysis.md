...
2. `ADAPT-003`: evolve the new misconception signals from the current richer taxonomy, recurrence-aware profile compaction, and confidence scoring into a stronger learned classifier or at least a stronger per-KC disambiguation layer.
...
4. `ADAPT-005` + `LLM-002`: deepen the new learner-strategy layer beyond the current heuristic recovery, plateau, relapse, and volatility profiles so the Socratic-to-profile feedback loop can model misconception recurrence and stronger per-KC sequencing across longer spans instead of mostly summarizing support direction.
...
- deepen the learner-strategy and misconception-recurrence layers beyond the current recovery, plateau, relapse, volatility, and recurrence heuristics
