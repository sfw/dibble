...
class RouterCalibrationSignalService:
    ...
    def signal_for(self, *, student_id: UUID, request: GenerationRequest) -> RouteCalibrationSummary:
        ...
        assessment_events = [
            event for event in events if event.event_type == "assessment.socratic" and event.student_id == student_id
        ]
        ...
        average_run_outcome_score = round(
            sum(sample.run_summary_score or 0.0 for sample in stable_samples) / len(stable_samples),
            2,
        )
        average_confidence = round(
            sum(sample.run_calibration_confidence for sample in stable_samples) / len(stable_samples),
            2,
        )
        positive_run_rate = round(
            sum(1 for sample in stable_samples if sample.run_calibration_signal == "positive") / len(stable_samples),
            2,
        )
        negative_run_rate = round(
            sum(1 for sample in stable_samples if sample.run_calibration_signal == "negative") / len(stable_samples),
            2,
        )
        return RouteCalibrationSummary(
