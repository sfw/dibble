...
class GenerationModeCalibrator:
    ...
    socratic_conversation_signal_service: SocraticConversationSignalService | None = (
        None
    )
    ...
    def calibration_for(
    ...
    ) -> GenerationModeCalibration | None:
        ...
        state_profile = (
            self.state_signal_service.state_for(
                student_id=request.student_id, request=request
            )
        ...
        trait_profile = (
            self.trait_profile_signal_service.latest_for_student(
        ...
        socratic_profile = (
            self.socratic_conversation_signal_service.summary_for(
                student_id=request.student_id, request=request
            )
        ...
        matched_run_count = (
            signal.matched_run_count
            if signal.source != "insufficient"
            else strategy.matched_run_count
        )
        progress_signal = (
            signal.progress_signal
            if signal.source != "insufficient"
            else strategy.progress_signal
        )
        progress_delta = (
            signal.progress_delta
            if signal.source != "insufficient"
        ...
        return GenerationModeCalibration(
            ...
            sequence_source=session.source
    ...
    def _support_bias(
        ...
        *,
        ...
        socratic_bias = self._socratic_profile_support_bias(
            socratic_profile=socratic_profile
        )
    ...
    def _calibration_signal(
        ...
        *,
        ...
        if (
            ...
            and state_profile.signal != "insufficient"
        ):
    ...
    def _sequence_for(self, *, request, strategy_sequence, session):
        ...
        return strategy_sequence.model_copy(
            update={
                ...
                "primary_kc_id": session.primary_kc_id
                or strategy_sequence.primary_kc_id,
        ...
        *,
        ...
        if self._is_decisive_session(session):
            if session.arc_action == "reprobe_new_angle":
                return "Recent same-session support has started to loop, so the next generated step should change representation or reasoning frame instead of adding another similar scaffold."
            if session.arc_action == "bridge_with_target":
                return "Recent same-session recovery looks stable enough for one guided target application before a full transfer check."
            if session.arc_action == "restate_then_apply":
                return "Recent same-session recovery is consolidating, so the next generated step should ask for a brief restatement and one guided application."
            if session.socratic_steering_action == "repair_then_model":
                return "Recent Socratic turns still point to prerequisite repair, so the next generated step should model the correction before expecting freer explanation."
            if session.socratic_steering_action == "restate_then_apply":
                return "Recent Socratic turns show the learner has repaired the idea but still needs to restate it clearly and apply it once before a freer transfer move."
            if session.socratic_steering_action == "clarify_then_check":
                return "Recent Socratic turns exposed a narrow reasoning gap, so the next generated step should clarify that language and quickly check the learner's explanation."
            if session.socratic_steering_action == "verify_transfer":
                return "Recent Socratic turns demonstrated understanding, so the next generated step should shift from explanation toward independent transfer."
        ...
        if support_bias > 0 and signal.progress_signal == "improving":
            return "Recent matching runs have been improving across sessions, so the generation mode can allow slightly more independence."
        if support_bias < 0 and signal.progress_signal == "declining":
            return "Recent matching runs have been declining across sessions, so the generation mode should add one step of modeled support."
        if support_bias > 0:
            return "Recent matching runs stayed durably positive, so the generation mode can allow slightly more independence."
        if support_bias < 0:
            return "Recent matching runs trended negative, so the generation mode should add one step of modeled support."
        ...
        if (
            ...
            return "Durable learner-state evidence still shows reliable overload risk, so the next generation step should keep support explicit instead of fading too quickly."
        if (
            state_profile.source != "insufficient"
            and support_bias > 0
            ...
            return "Durable learner-state evidence shows stable recovery, so the next generation step can release one more move to the learner without overcommitting."
        if (
            ...
            return "Durable cognitive-trait evidence suggests challenge tolerance is still fragile, so the next step should keep the comparison space tight and the learner release small."
        if (
            trait_profile.source != "insufficient"
            and support_bias > 0
            ...
            return "Durable cognitive-trait evidence suggests the learner can handle a lighter release, so the next step can shift toward transfer with minimal cueing."
        if (
            session.current_evidence_signal == "productive_struggle"
            and session.current_evidence_confidence >= 0.58
        ):
            return "Current observation evidence looks like productive struggle rather than overload, so the next step should preserve challenge while keeping support targeted."
        if (
            session.current_evidence_signal == "overload"
            and session.current_evidence_confidence >= 0.58
        ):
            return "Current observation evidence looks like reliable overload, so the next generation step should keep support explicit instead of treating the learner's friction as healthy challenge."
        if (
            session.current_evidence_signal == "disengagement"
            and session.current_evidence_confidence >= 0.58
        ):
            return "Current observation evidence looks more like disengagement than productive challenge, so the next generation step should re-engage the learner before adding independence."
        if (
            session.current_evidence_signal == "support_dependence"
            and session.current_evidence_confidence >= 0.58
        ):
            return "Current observation evidence shows the learner succeeding mainly under heavy support, so the next generation step should tighten scaffolds instead of releasing a bigger transfer move."
        return "Recent matching runs were informative but not decisive enough to override the baseline mode heuristics."

    def _current_evidence_support_bias(
        self, *, session, state_profile, trait_profile
    ) -> int:
        if session.current_evidence_confidence < 0.58:
            return 0
        if session.current_evidence_signal in {
            "overload",
            "disengagement",
            "support_dependence",
        }:
    ...
    def _durable_profile_support_bias(self, *, state_profile, trait_profile) -> int:
        ...
        if state_profile.source != "insufficient":
            if (
                state_profile.load_reliability >= 0.58
                and state_profile.overload_risk >= 0.64
            ):
                support_pressure += 2
            if (
    ...
    def _socratic_profile_support_bias(self, *, socratic_profile) -> int:
        if (
            socratic_profile.source == "insufficient"
            or socratic_profile.confidence < 0.58
        ):
            return 0
        if socratic_profile.signal == "independent_check":
            return 1
