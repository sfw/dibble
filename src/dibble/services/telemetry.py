...
class TelemetryService:
    ...
    def snapshot(self) -> TelemetrySnapshot:
        ...
        decision_events = [
            event for event in events if event.event_type == "adaptive.decide"
        ]
        socratic_events = [
        ...
        warm_events = [
            event
            for event in events
            if event.event_type in {"content.warm", "content.warm.predictive"}
        ]
        moderation_events = [
            event for event in events if event.event_type == "content.moderation"
        ]
        predictive_warm_events = [
            event for event in events if event.event_type == "content.warm.predictive"
        ]
        predictive_warm_process_events = [
            event
            for event in events
            if event.event_type == "content.warm.predictive.process"
        ]
        cache_invalidation_events = [
            event for event in events if event.event_type == "content.cache.invalidate"
        ]
        provider_events = (
            self.provider_health_store.list(limit=500)
            if self.provider_health_store is not None
            else []
        )
        cache_stats = (
            self.generated_content_store.stats()
            if self.generated_content_store is not None
            else {
                "total_entries": 0,
                "fresh_entries": 0,
                "expired_entries": 0,
        ...
        generation_prompt_groups: dict[
            tuple[str, str | None, str | None], list[object]
        ] = {}
        observation_events = [
        ...
        for event in generation_events:
            ...
            key = (
                str(template_name),
                str(event.payload.get("prompt_template_variant"))
                if event.payload.get("prompt_template_variant")
                else None,
                str(event.payload.get("content_type"))
                if event.payload.get("content_type")
                else None,
            )
        ...
        prompt_performance_groups: dict[
            tuple[str, str | None, str | None], list[object]
        ] = {}
        for event in socratic_events:
            template_name = event.payload.get("prompt_template_name")
            ...
            key = (
                ...
                if event.payload.get("prompt_style")
                else None,
            )
        ...
        return TelemetrySnapshot(
            ...
            improving_progress_signals=sum(
                1
                for event in progress_profile_events
                if event.payload.get("progress_signal") == "improving"
            ),
            declining_progress_signals=sum(
                1
                for event in progress_profile_events
                if event.payload.get("progress_signal") == "declining"
            ),
            socratic_profile_updates=sum(
                1
                for event in socratic_events
                if bool(event.payload.get("profile_update_applied"))
            ),
            socratic_demonstrated_events=sum(
                1
                for event in socratic_events
                if event.payload.get("evidence_strength") == "demonstrated"
            ),
            socratic_step_back_events=sum(
                1
                for event in socratic_events
                if event.payload.get("next_action") == "step_back"
            ),
            average_socratic_evidence_score=(
                round(sum(socratic_evidence_scores) / len(socratic_evidence_scores), 2)
                if socratic_evidence_scores
                else 0.0
            ),
            fallback_generations=sum(
                1
                for event in generation_events
                if event.payload.get("delivery_mode") == "static_fallback"
            ),
            moderation_events=len(moderation_events),
            moderation_stream_events=sum(
                1
                for event in moderation_events
            ...
            moderation_blocked_requests=sum(
                1
                for event in moderation_events
                if event.payload.get("stage") == "request"
                and bool(event.payload.get("blocked"))
            ),
            moderation_rewritten_responses=sum(
                1
                for event in moderation_events
                if event.payload.get("stage") == "response"
                and bool(event.payload.get("response_rewritten"))
            ),
            moderation_provider_bypass_events=sum(
                1
                for event in moderation_events
                if bool(event.payload.get("request_blocked"))
                and not bool(event.payload.get("provider_invoked"))
            ),
            ...
            validation_issue_events=sum(
                1
                for event in generation_events
                if int(event.payload.get("validation_issue_count", 0)) > 0
            ),
            cache_hit_generations=sum(
                1 for event in generation_events if bool(event.payload.get("cache_hit"))
            ),
            warm_requests=sum(
                int(
                    event.payload.get(
                        "total_requests",
                        event.payload.get("predicted_request_count", 0),
                    )
                )
            ...
            predictive_warm_requests=sum(
                int(event.payload.get("predicted_request_count", 0))
                for event in predictive_warm_events
            ),
            predictive_warm_process_events=len(predictive_warm_process_events),
            predictive_cache_invalidations=sum(
                int(event.payload.get("expired_entries", 0))
            ...
            supplemental_inline_predictive_warm_tasks=sum(
                int(event.payload.get("supplemental_tasks", 0))
                for event in predictive_warm_events
            ),
            ...
            stale_processing_predictive_warm_tasks=queue_stats.get(
                "stale_processing", 0
            ),
            urgent_predictive_warm_tasks=queue_stats.get("urgent_active", 0),
            next_predictive_warm_task_eta_seconds=queue_stats.get(
                "next_eligible_in_seconds"
            ),
            ...
            retried_predictive_warm_tasks=sum(
                int(event.payload.get("retried_tasks", 0))
                for event in predictive_warm_process_events
            ),
            requeued_predictive_warm_tasks=sum(
                int(event.payload.get("requeued_tasks", 0))
                for event in predictive_warm_process_events
            ),
            dropped_predictive_warm_tasks=sum(
                int(event.payload.get("dropped_tasks", 0))
                for event in predictive_warm_process_events
            ),
            generated_content_entries=cache_stats["total_entries"],
            fresh_generated_content_entries=cache_stats["fresh_entries"],
            provider_failure_events=sum(
                1 for event in provider_events if event.status == "failure"
            ),
            provider_circuit_open_events=sum(
                1 for event in provider_events if event.status == "circuit_open"
            ),
    ...
    def _build_socratic_prompt_performance(
        ...
        evidence_scores = [
            float(event.payload.get("evidence_score", 0.0)) for event in events
        ]
        ...
        profile_updates = sum(
            1 for event in events if bool(event.payload.get("profile_update_applied"))
        )
        return SocraticPromptPerformance(
            ...
            average_evidence_score=round(sum(evidence_scores) / event_count, 2)
            if event_count
            else 0.0,
            demonstrated_rate=round(demonstrated_events / event_count, 2)
            if event_count
            else 0.0,
            profile_update_rate=round(profile_updates / event_count, 2)
            if event_count
            else 0.0,
        )
    ...
    def _build_generation_prompt_performance(
        ...
        run_scores = [
            sample.run_summary_score
            for sample in samples
            if sample.run_summary_score is not None
        ]
        run_summary_matches = sum(
            1 for sample in samples if sample.run_summary_score is not None
        )
        persisted_run_summary_matches = sum(
            1 for sample in samples if sample.run_summary_source == "persisted"
        )
        positive_run_signals = sum(
            1 for sample in samples if sample.run_calibration_signal == "positive"
        )
        run_signal_confidence_total = sum(
            sample.run_calibration_confidence for sample in samples
        )
        downstream_matches = sum(
            1 for sample in samples if sample.downstream_observation_score is not None
        )
        assessment_matches = sum(
            1 for sample in samples if sample.downstream_assessment_score is not None
        )
        session_outcome_matches = sum(
            1 for sample in samples if sample.session_outcome_score is not None
        )
        observation_trace_total = sum(
            sample.observation_match_count for sample in samples
        )
        assessment_trace_total = sum(
            sample.assessment_match_count for sample in samples
        )
        session_generation_depth_total = sum(
            sample.session_generation_depth for sample in samples
        )
        return GenerationPromptPerformance(
            ...
            event_count=event_count,
            average_quality_score=round(sum(quality_scores) / event_count, 2)
            if event_count
            else 0.0,
            average_composite_outcome=round(sum(composite_scores) / event_count, 2)
            if event_count
            else 0.0,
            average_run_outcome_score=round(sum(run_scores) / len(run_scores), 2)
            if run_scores
            else 0.0,
            average_run_signal_confidence=round(
                run_signal_confidence_total / event_count, 2
            )
            if event_count
            else 0.0,
            run_summary_rate=round(run_summary_matches / event_count, 2)
            if event_count
            else 0.0,
            persisted_run_summary_rate=round(
                persisted_run_summary_matches / event_count, 2
            )
            if event_count
            else 0.0,
            positive_run_signal_rate=round(positive_run_signals / event_count, 2)
            if event_count
            else 0.0,
            downstream_observation_rate=round(downstream_matches / event_count, 2)
            if event_count
            else 0.0,
            downstream_assessment_rate=round(assessment_matches / event_count, 2)
            if event_count
            else 0.0,
            session_outcome_rate=round(session_outcome_matches / event_count, 2)
            if event_count
            else 0.0,
            average_observation_trace_count=round(
                observation_trace_total / event_count, 2
            )
            if event_count
            else 0.0,
            average_assessment_trace_count=round(
                assessment_trace_total / event_count, 2
            )
            if event_count
            else 0.0,
            average_session_generation_depth=round(
                session_generation_depth_total / event_count, 2
            )
            if event_count
            else 0.0,
        )
