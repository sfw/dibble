...
class LearningProgressProfileSnapshot:
    ...
    recent_average_run_outcome_score: float | None = None
    prior_average_run_outcome_score: float | None = None
    progress_delta: float = 0.0
    progress_signal: str = "insufficient"


@dataclass(slots=True)
class LearningProgressProfileBuilder:
    recency_window_days: int = 30
    max_matched_runs: int = 12
    recent_window_size: int = 3
    minimum_confidence_for_signal: float = 0.55
    minimum_session_count_for_signal: int = 2
    ...
    def build_from_summary_event(
        ...
        *,
        ...
        return LearningProgressProfileSnapshot(
            ...
            recent_average_run_outcome_score=recent_average,
            prior_average_run_outcome_score=prior_average,
    ...
    def _progress_signal(
        ...
        *,
        ...
        if prior_average is None:
            return "tentative"
        if average_run_confidence < self.minimum_confidence_for_signal:
            return "tentative"
        if matched_session_count < self.minimum_session_count_for_signal:
            return "tentative"
        if progress_delta >= self.trend_delta_threshold:
            return "improving"
        if progress_delta <= -self.trend_delta_threshold:
            return "declining"
        return "stable"
            ...
            recorded.append(
                self.audit_store.append(
                    ...
                    payload={
                        ...
                        "recent_average_run_outcome_score": snapshot.recent_average_run_outcome_score,
                        "prior_average_run_outcome_score": snapshot.prior_average_run_outcome_score,
...
@dataclass(slots=True)
class LearningProgressProfileResolver:
    recency_window_days: int = 30
