...
class WithinSessionAdaptationService:
    ...
    def record_observation_event(
        self, *, student_id: UUID, event_payload: dict[str, object]
    ) -> WithinSessionAdaptationSummary:
        request = self._request_from_payload(
            student_id=student_id, payload=event_payload, default_intent="practice"
        )
        if request is None:
            return WithinSessionAdaptationSummary()
    ...
    def record_assessment_event(
        ...
        request = self._request_from_payload(
            student_id=student_id, payload=event_payload, default_intent="assessment"
        )
        if request is None:
            return WithinSessionAdaptationSummary()
        raw_summary = self._raw_adaptation_for(student_id=student_id, request=request)
    ...
    def record_generation_step(
        ...
        *,
        ...
        if existing is None:
            raw_summary = self._raw_adaptation_for(
            ...
            existing = self._build_controller_state(
                ...
                recovery_intent=self._recovery_intent_for(
                    raw_summary=raw_summary, existing=None
                ),
        ...
        updated = existing.model_copy(
            update={
                ...
                "support_steps_remaining": self._remaining_steps_after_generation(
                    existing
                ),
    ...
    def _raw_adaptation_for(
        ...
        latest_assessment_payload = (
            latest_assessment.payload if latest_assessment is not None else None
        )
        latest_assessment_prompt_style = (
            str(latest_assessment_payload.get("prompt_style"))
            if latest_assessment_payload is not None
            and latest_assessment_payload.get("prompt_style") is not None
            else None
        )
        ...
        socratic_steering_action = self._socratic_steering_action(
            latest_assessment_payload
        )

        for event in observation_events:
            observation_negative, observation_positive = self._observation_scores(
                event.payload
            )
        ...
        for event in assessment_events:
            assessment_negative, assessment_positive = self._assessment_scores(
                event.payload
            )
        ...
        confidence = round(
            min(
                0.92,
                0.46
                + (len(observation_events) * 0.16)
                + (len(assessment_events) * 0.22),
            ),
            2,
        )
        primary_kc_id = self._primary_kc_id(request=request, events=events)
        strong_assessment_recovery = self._strong_assessment_recovery(
            latest_assessment.payload if latest_assessment is not None else None
        )
        (
        ...
        ) = self._current_evidence_summary(observation_events)
    ...
    def _matches_request(
        ...
        if (
            ...
            and self._overlap_score(request.target_kc_ids, payload.get("target_kc_ids"))
            > 0.0
        ):
            return True
        if (
            request.target_lo_ids
            and self._overlap_score(request.target_lo_ids, payload.get("target_lo_ids"))
            > 0.0
        ):
            return True
    ...
    def _observation_scores(self, payload: dict[str, object]) -> tuple[float, float]:
        ...
        current_evidence_confidence = float(
            payload.get("current_evidence_confidence", 0.0)
        )
    ...
    def _strong_assessment_recovery(
        self, latest_assessment: dict[str, object] | None
    ) -> bool:
        if latest_assessment is None:
            return False
        evidence_strength = str(
            latest_assessment.get("evidence_strength", "insufficient")
        )
    ...
    def _negative_rationale(
        ...
        *,
        ...
        if (
            ...
            and latest_assessment.get("next_action") == "step_back"
        ):
    ...
    def _positive_rationale(
        ...
        *,
        ...
        if (
            latest_assessment is not None
            and latest_assessment.get("next_action") == "advance"
        ):
            return decision_grade_rationale(
    ...
    def _controller_for(
        ...
        if (
            request.target_kc_ids
            and self._overlap_score(request.target_kc_ids, controller.target_kc_ids)
            <= 0.0
        ):
            return None
        if (
            request.target_lo_ids
            and self._overlap_score(request.target_lo_ids, controller.target_lo_ids)
            <= 0.0
        ):
            return None
        return controller

    def _summary_from_controller(
    ...
    def _transition_controller(
        ...
        *,
        ...
        existing = self.controller_store.get(request.learning_session_id)
        positive_streak = self._updated_streak(
            existing.positive_streak if existing is not None else 0,
            raw_summary.signal == "positive",
        )
        negative_streak = self._updated_streak(
            existing.negative_streak if existing is not None else 0,
            raw_summary.signal == "negative",
        )
        mixed_streak = self._updated_streak(
            existing.mixed_streak if existing is not None else 0,
            raw_summary.signal == "mixed",
        )
        phase = self._phase_for(raw_summary=raw_summary, existing=existing)
        recovery_intent = self._recovery_intent_for(
            raw_summary=raw_summary, existing=existing
        )
        phase_changed = existing is None or existing.phase != phase
        support_step_budget = self._support_step_budget_for(
            phase=phase, raw_summary=raw_summary
        )
        ...
        updated = WithinSessionControllerState(
            ...
            or (existing.primary_kc_id if existing is not None else None),
            phase=phase,
            ...
            last_generated_content_type=existing.last_generated_content_type
            if existing is not None
            else None,
            last_generation_id=existing.last_generation_id
            if existing is not None
            else None,
    ...
    def _support_step_budget_for(
        self, *, phase: str, raw_summary: WithinSessionAdaptationSummary
    ) -> int:
        base_budget = {
            "stabilize": 1,
            "repair": 2,
        ...
        if (
            phase in {"stabilize", "repair"}
            and raw_summary.latest_assessment_next_action == "step_back"
        ):
    ...
    def _support_steps_remaining_for(
        ...
        *,
        ...
        if (
            raw_summary.signal == "positive"
            and phase in {"consolidate", "bridge"}
            and existing.signal in {"negative", "mixed", "recovering"}
        ):
            return support_step_budget
        return min(support_step_budget, existing.support_steps_remaining)

    def _remaining_steps_after_generation(
        self, controller: WithinSessionControllerState
    ) -> int:
    ...
    def _stuck_loop_risk_for(
        ...
        *,
        ...
        if not self._tracks_support_budget(phase):
            return "low"
        if support_steps_remaining <= 0 and (
            generation_count >= 2
    ...
    def _controller_confidence(
        ...
        *,
        ...
        phase_bonus = (
            0.04
            if phase in {"repair", "consolidate", "bridge", "transfer_check"}
            else 0.0
        )
        loop_penalty = (
            0.08
        ...
        return round(
            max(
                ...
                min(
                    0.95,
                    raw_summary.confidence + streak_bonus + phase_bonus - loop_penalty,
                ),
            ),
            2,
        )
    ...
    def _controller_rationale(
        ...
        *,
        ...
        if raw_summary.signal == "positive" and self._current_evidence_blocks_transfer(
            raw_summary
        ):
            return decision_grade_rationale(
    ...
    def _generation_step_rationale(
        ...
        *,
        ...
        return f"Within-session controller for {learning_session_id} has delivered {generation_count} generated steps in the current {existing.phase} arc."

    def _current_evidence_blocks_transfer(
        self, raw_summary: WithinSessionAdaptationSummary
    ) -> bool:
