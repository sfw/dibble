...
class PredictiveContentInvalidator:
    ...
    def invalidate_from_trigger_event(self, trigger_event: AuditEvent) -> AuditEvent:
        expired_entries = self.generated_content_store.expire_predictive_content(
            ...
            if trigger_event.student_id is not None
            else None,
            learning_session_id=_string_or_none(
                trigger_event.payload.get("learning_session_id")
            ),
        ...
        return self.audit_store.append(
            ...
            payload={
                ...
                "target_kc_ids": _string_list(
                    trigger_event.payload.get("target_kc_ids")
                ),
                "target_lo_ids": _string_list(
                    trigger_event.payload.get("target_lo_ids")
                ),
