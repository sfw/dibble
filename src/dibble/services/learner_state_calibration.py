...
class LearnerStateCalibrator:
    ...
    def calibrate(
        ...
        *,
        ...
        request = self._request_from_observation(
            student_id=student_id, observation=observation
        )
        state_profile = (
        ...
        if self._should_apply_state_profile(
            ...
            return LearnerStateCalibrationResult(
                ...
                current_evidence_signal=(
                    current_evidence.signal
                    if current_evidence is not None
                    else "steady"
                ),
                current_evidence_confidence=(
                    current_evidence.confidence if current_evidence is not None else 0.0
                ),
                current_evidence_rationale=(
                    current_evidence.rationale if current_evidence is not None else None
                ),
        ...
        if (
            ...
            and signal.confidence >= self.positive_confidence_threshold
        ):
            return LearnerStateCalibrationResult(
                    ...
                    update={
                        "metacognitive_state": self._adjust_metacognitive_state(
                            ...
                            confidence_delta=min(
                                0.12, 0.05 + (signal.confidence * 0.08)
                            ),
                            self_monitoring_delta=min(
                                0.1, 0.04 + (signal.confidence * 0.07)
                            ),
                            help_seeking_shift=-1,
                            effectiveness_delta=min(
                                0.08, 0.03 + (signal.confidence * 0.05)
                            ),
                        )
                    }
                ),
                ...
                overload_risk=self._observation_strain(
                    observation=observation, state=inferred_state
                ),
                current_evidence_signal=(
                    current_evidence.signal
                    if current_evidence is not None
        ...
        if (
            ...
            and signal.confidence >= self.negative_confidence_threshold
        ):
            return LearnerStateCalibrationResult(
                state=inferred_state.model_copy(
                    update={
                        "metacognitive_state": self._adjust_metacognitive_state(
                            ...
                            self_monitoring_delta=-min(
                                0.08, 0.03 + (signal.confidence * 0.06)
                            ),
                            help_seeking_shift=1,
                            effectiveness_delta=-min(
                                0.06, 0.02 + (signal.confidence * 0.04)
                            ),
                        )
                    }
                ),
        ...
        return LearnerStateCalibrationResult(
            ...
            current_evidence_signal=(
                current_evidence.signal if current_evidence is not None else "steady"
            ),
            current_evidence_confidence=(
                current_evidence.confidence if current_evidence is not None else 0.0
            ),
            ...
            update={
                ...
                "help_seeking_effectiveness": round(
                    min(
                        1.0,
                        max(
                            0.0, state.help_seeking_effectiveness + effectiveness_delta
                        ),
                    ),
                    2,
                ),
                "help_seeking": self._shift_signal_level(
                    state.help_seeking, help_seeking_shift
                ),
            }
        )
    ...
    def _should_apply_state_profile(
        ...
        *,
        ...
        if profile.confidence < self.state_profile_confidence_threshold:
            return False
        observation_strain = self._observation_strain(
            observation=observation, state=inferred_state
        )
        current_evidence = inferred_state.current_evidence
        ...
        if profile.signal == "support_needed":
            return not self._current_evidence_blocks_support(
                current_evidence, observation_strain=observation_strain
            ) and (
    ...
    def _current_evidence_blocks_release(self, current_evidence) -> bool:
        if current_evidence is None or current_evidence.confidence < 0.58:
            return False
        return current_evidence.signal in {
            "overload",
            "disengagement",
            "support_dependence",
        }

    def _current_evidence_blocks_support(
        self, current_evidence, *, observation_strain: float
    ) -> bool:
        if current_evidence is None or current_evidence.confidence < 0.62:
            return False
        return (
            current_evidence.signal == "productive_struggle"
            and observation_strain < 0.52
        )
    ...
    def _blend_with_state_profile(
        ...
        *,
        ...
        affective_weight = min(
            0.5, max(0.12, weight + (profile.affective_reliability * 0.12) - 0.04)
        )
        load_weight = min(
            0.54, max(0.14, weight + (profile.load_reliability * 0.14) - 0.03)
        )
                    ...
                    update={
                        ...
                        "help_seeking_effectiveness": round(
                            self._blend_numeric(
                                ...
                                target=self._profile_help_seeking_effectiveness(
                                    profile
                                ),
                                weight=metacognitive_weight,
                            ),
    ...
    def _state_profile_weight(
        ...
        *,
        ...
        if profile.signal == "support_needed":
            ...
            weight += (profile.recovery_stability * 0.12) + (
                profile.metacognitive_reliability * 0.08
            )
            weight -= observation_strain * 0.18
        else:
            weight += (profile.recovery_stability * 0.1) - abs(
                observation_strain - 0.45
            ) * 0.08
    ...
    def _observation_strain(
        ...
        *,
        ...
        strain += (
            0.12
            if state.affective_state.frustration
            in {SignalLevel.medium, SignalLevel.high}
            else 0.0
        )
    ...
    def _blend_signal_level(
        self, current: SignalLevel, target: SignalLevel, *, weight: float
    ) -> SignalLevel:
        ordered = [
            SignalLevel.none,
            SignalLevel.low,
    ...
    def _profile_help_seeking_effectiveness(
        self, profile: LearnerStateProfileSummary
    ) -> float:
        if profile.signal == "support_needed":
            return max(0.25, profile.confidence_calibration - 0.05)
        if profile.signal == "independence_ready":
