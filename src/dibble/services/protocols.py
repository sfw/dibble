...
class CurriculumSnapshotDiffStore(Protocol):
...
class CurriculumImpactAnalysisStore(Protocol):
    def upsert(
        self, analysis: CurriculumImpactAnalysis
    ) -> CurriculumImpactAnalysis: ...
...
class CurriculumMigrationPlanStore(Protocol):
