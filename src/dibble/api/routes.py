...
from dibble.api.guardian_routes import build_guardian_router
from dibble.api.household_routes import build_household_router
...
from dibble.api.placement_routes import build_placement_router
from dibble.api.session_routes import build_session_router
...
from dibble.models.setup import DeploymentReadiness
...
def build_router(services: ApiServices) -> APIRouter:
    ...
    @router.get("/ready", response_model=DeploymentReadiness)
    def readiness() -> DeploymentReadiness:
        return context.services.setup_config_service.get_deployment_readiness()
    ...
    router.include_router(build_guardian_router(context))
    router.include_router(build_household_router(context))
    ...
    router.include_router(build_placement_router(context))
    router.include_router(build_session_router(context))
