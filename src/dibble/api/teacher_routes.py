...
from dibble.models.teacher_classroom import TeacherClassroomOverview, TeacherClassroomReadModel
...
def build_teacher_router(context: ApiContext) -> APIRouter:
    ...
    @router.put("/classrooms/{classroom_id}", response_model=Classroom, dependencies=context.deps("editor"))
    ...
    @router.get("/classrooms", response_model=list[TeacherClassroomOverview], dependencies=context.deps("viewer"))
    def list_classrooms() -> list[TeacherClassroomOverview]:
        return services.teacher_classroom_service.list_classrooms(services.classroom_store.list())
