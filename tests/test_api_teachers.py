...
def test_teacher_classroom_read_model_packages_learner_cards_and_counts(
    ...
    # Create classroom and users via API first (learner_id is auto-generated).
    ...
    # Discover auto-generated learner_ids via GET.
    active_user = client.get(
        f"/api/users/{active_create.json()['user_id']}"
    ).json()
    blocked_user = client.get(
        f"/api/users/{blocked_create.json()['user_id']}"
    ).json()
    active_student_id = active_user["learner_id"]
    blocked_student_id = blocked_user["learner_id"]

    # Now create profiles and resources using the discovered learner_ids.
    client.put(
    ...
    active_card = next(
        ...
        if card["student_id"] == active_student_id
    )
    blocked_card = next(
        card
        for card in classroom_payload["learners"]
        if card["student_id"] == blocked_student_id
    )
...
def test_teacher_classroom_resolves_learners_from_user_enrollments(client):
    # Create user first; learner_id is auto-generated by the API.
    client.put(
    ...
    # Discover auto-generated learner_id via GET.
    learner_user = client.get(
        f"/api/users/{create_learner.json()['user_id']}"
    ).json()
    student_id = learner_user["learner_id"]

    # Create profile using the discovered learner_id.
    client.put(
        f"/api/learners/{student_id}/profile",
        json=build_profile(student_id),
    )
    ...
    assert classroom_payload["learners"][0]["student_id"] == student_id
    assert trends_payload["learner_count"] == 1
    assert trends_payload["learner_trends"][0]["student_id"] == student_id
