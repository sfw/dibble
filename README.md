...
- The ordinary generation path now also applies a small mastery gate when same-session evidence still says hold on the current target, so premature assessment-style requests can be rewritten into target-aligned practice with explicit requested-versus-applied content-type metadata instead of silently attempting transfer too early
...
3. Broaden mastery-loop and progression ownership beyond the current local hold, redirection, and assessment-gate heuristics.
...
5. Expand predictive warming beyond the current queue-backed inline scheduler into a broader background execution path with stronger ownership and invalidation signals.
