...
- The misconception pipeline now also compacts repeated remediation signals into richer `learning.misconception.profile` events with recurrence counts, session counts, and recurrence labels such as `recurring` or `relapsing`, and those durable signals now feed later misconception detection and remediation blueprint selection
...
4. Deepen the learner-strategy and misconception-recurrence layers beyond the current heuristic recovery, plateau, relapse, volatility, and recurrence signals so they can make stronger per-KC sequencing and misconception-disambiguation decisions.
