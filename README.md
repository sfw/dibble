...
- The backend now also compacts recent run summaries, progress profiles, and learner-strategy profiles into durable `learning.state.profile` events, and the learner observation pipeline can blend those cross-session state targets back into live affective, load, and metacognitive updates instead of relying only on the latest observation window
...
- The backend now also compacts recent learner observations plus durable state-profile context into `learning.cognitive_trait.profile` events, and live cognitive-trait inference can blend those cross-session trait targets back into processing-speed, working-memory, and spatial-reasoning updates instead of relying only on the current observation batch
...
3. Deepen the new durable learner-state and cognitive-trait profiles beyond the current cross-session affective/load/metacognitive/trait calibration so routing and content-mode selection rely less on hand-tuned heuristics.
