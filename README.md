...
- The backend now also compacts recent matching run summaries into cross-session `learning.strategy.profile` audit events, producing honest heuristic recovery and independence signals that routing, generation-mode calibration, remediation workflows, and learner summaries can reuse directly
...
- Remediation is now session-backed, so `POST /api/remedial/trigger` starts a persisted multi-step workflow and later steps can be reloaded or advanced through dedicated remediation-session endpoints, and those sessions now carry learner-strategy context so repeated struggle can keep prerequisite rebuild guidance explicit across steps
...
4. Deepen the new learner-strategy profile beyond the current heuristic recovery/independence signals so it can model richer plateau, volatility, and misconception-recurrence patterns.
