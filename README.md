...
- Teacher classroom learner cards now carry a backend-owned `display_rationale` field computed server-side, so the frontend can show one canonical rationale without selecting among multiple fallback sources
- Progression decisions now produce validated outcome signals: a `ProgressionOutcomeTracker` evaluates recent hold, transfer, and prerequisite-rebuild decisions against subsequent mastery evidence and records `progression.outcome` audit events so the system can measure whether its own heuristics are actually helping
- Progression outcome verdicts now feed back into live decisions with recency weighting: a `ProgressionOutcomeSignalService` aggregates recent outcome verdicts with time-decay weighting into per-action reliability signals with hold-subtype granularity (hold_target vs hold_repair_target vs hold_bridge_target tracked separately) plus prerequisite rebuild threshold adjustments, and `ProgressionOwnershipService` adjusts hold confidence thresholds and transfer confidence requirements based on whether prior decisions actually helped the learner
- Completed remediation sessions now produce misconception resolution outcomes: a `MisconceptionRemediationOutcomeTracker` evaluates recently completed remediation sessions against subsequent mastery and struggle evidence and records `misconception.remediation.outcome` audit events with resolved, unresolved, or inconclusive verdicts
- Misconception remediation outcomes now feed back into detection confidence: a `MisconceptionRemediationOutcomeSignalService` aggregates per-KC remediation outcomes with recency weighting and feeds confidence adjustments back into the `MisconceptionDetector`, boosting confidence for persistent misconceptions and slightly tempering it for consistently-resolved ones, with an explicit `persistent_misconception` flag when repeated failures suggest the current approach needs teacher review
- Curriculum progression now has a mastery quality gate (ADAPT-006): resources are not marked as mastered when their KCs show `support_dependent` or `fragile` ordinary mastery signals with sufficient confidence, even when raw mastery scores exceed the mastery threshold, so dependents do not unlock prematurely when the learner reached high scores through heavy scaffolding or volatile practice
- Resource state transitions are now tracked as `curriculum.resource.transition` audit events (ADAPT-006 + ORCH-001): the backend records when resources change state (ready→active→mastered, or regressions like mastered→ready via decay or quality gate), including quality gate metadata, so downstream outcome validation and teacher surfaces can inspect the curriculum progression audit trail
- The mastery quality gate now has an outcome feedback loop (ADAPT-006): a `MasteryQualityGateOutcomeTracker` evaluates whether gate holds actually helped learners (mastery improvement after hold vs stalled progress), records `mastery_quality_gate.outcome` audit events with positive/negative/inconclusive verdicts, and a `MasteryQualityGateSignalService` aggregates those outcomes with recency weighting to feed bounded confidence threshold adjustments back into the quality gate so it can self-correct — gating more readily when holds consistently help and backing off when they consistently stall
...
The observation pipeline now also validates whether learner-state predictions (productive struggle, overload, disengagement, support dependence) are confirmed or contradicted by subsequent learning outcomes, recording `learner_state_prediction.outcome` audit events with per-classification accuracy verdicts, mastery deltas, and completion rate evidence. A companion signal service aggregates those outcomes with recency weighting into per-learner, per-classification reliability signals, surfaces overall and per-classification accuracy rates through the learner summary, and identifies the weakest and strongest classifications so the system and teachers can tell when state inference is trustworthy versus noisy. The frontend teacher LearnerDetail view now also renders model reliability when prediction outcomes are available, showing overall accuracy, per-classification breakdown, and rationale text with color-coded confidence indicators.
