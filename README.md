...
- Progression outcome verdicts now feed back into live decisions: a `ProgressionOutcomeSignalService` aggregates recent outcome verdicts into per-action reliability signals, and `ProgressionOwnershipService` adjusts hold confidence thresholds and transfer confidence requirements based on whether prior decisions actually helped the learner
