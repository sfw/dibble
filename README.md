...
- Same-session observations and Socratic assessments now feed a persisted within-session controller keyed by `learning_session_id`, so active sessions can carry explicit arc phases such as `stabilize`, `repair`, `consolidate`, `bridge`, and `transfer_check` plus recovery intent, streaks, and generated-step counts across requests while the next generation step raises or relaxes support and updates sequencing before the slower cross-session profiles catch up
...
- Recent Socratic assessment turns now also feed an explicit conversational steering layer, so the next generated step can react to recent `step_back`, `clarification`, or `transfer_check` outcomes by changing prompt family, guidance, and even the selected next content mode more directly
- Practice generation now hydrates lightweight target-KC misconception hints from the live KC catalog, so prompt guidance and fallback content can aim distractors at a named misconception plus corrective move instead of relying only on generic distractor styles
- Worked-example generation now carries named visible step roles, a hidden learner-owned step role, and a transfer move, so fading can be expressed as a role-based release plan rather than only a visible-step count
- Predictive warming now behaves more like a lightweight scheduler, with urgency classes, bounded batch selection so routine work is not starved forever, retry/backoff for transient failures, and richer queue/process telemetry
- The generation engine now runs an explicit moderation pass before and after provider generation, so unsafe learner prompts or unsafe generated drafts can be replaced with a teacher-safe fallback and surfaced with explicit moderation metadata instead of being treated as ordinary validation issues
- The backend now also compacts recent learner observations plus durable state-profile context into `learning.cognitive_trait.profile` events, including trait-stability and challenge-tolerance signals, and live cognitive-trait inference can blend those cross-session trait targets back into processing-speed, working-memory, and spatial-reasoning updates with stronger guards against over-trusting stale, weak, or sharply mismatched durable evidence
...
3. Deepen the new durable learner-state and cognitive-trait profiles beyond the current cross-session affective/load/metacognitive recovery, overload, stability, and challenge-tolerance calibration so routing and content-mode selection rely less on hand-tuned heuristics.
4. Deepen the new persisted within-session controller beyond the current phase, streak, and recovery-intent heuristics so it can coordinate richer multi-step arcs and recovery decisions inside an active session.
