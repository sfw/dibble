...
- `GET /api/learners/{student_id}/summary` now exposes a frontend-ready learner overview with engagement, metacognitive snapshot, latest calibration summary, and recent activity counts so the UI does not need to read audit logs directly
...
4. Extend the new persisted run-summary and calibration-profile layers into broader learner-history adaptation so the system can move from recent cross-session snapshots toward richer long-horizon signals and more predictive frontend-facing progress views.
