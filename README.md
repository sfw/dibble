...
- Same-session observations and Socratic assessments now feed a persisted within-session controller keyed by `learning_session_id`, so active sessions can carry phase, recovery intent, streaks, and generated-step counts across requests while the next generation step raises or relaxes support and updates sequencing before the slower cross-session profiles catch up
...
- The KC graph layer now has a reusable graph service for broader prerequisite/dependent traversal, distance-aware weighting, LO-to-KC backfill estimates, and weighted LO mastery recomputation, so Socratic mastery migration no longer treats every prerequisite hop or sibling KC as equally strong
