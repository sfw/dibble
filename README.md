...
3. Keep extending problem construction and worked-example transfer moves now that distractor families, fade roles, richer curriculum grounding, backend-owned target redirection, and cleaner current-evidence guardrails are in place.
4. Deepen moderation coverage and stream-time handling if stricter real-time safety becomes a stronger product constraint.
5. Replace more of the within-session controller's current heuristic support-budget and loop-risk logic with stronger outcome-backed policies once real session traces accumulate.
6. Expand predictive warming beyond the current rule-based follow-up planner into a broader scheduler with stronger invalidation signals and background execution options.
